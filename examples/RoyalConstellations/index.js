...
//Canvas
var canvasLinks = d3.select('#chart').append('canvas')
...
var royals = currentRoyalLeaders.map(function(d) {
  return d.id
})
...
var interestingRoyal = [
  {
    ...
    note: 'Killed by the guillotine during the French revolution'
  },
  {
    id: 'I37',
    country: 'Russia',
    title: 'Tsar of France',
    name: 'Nicholas II',
    note: 'The last Tsar of Russia'
  },
  {
    id: 'I1372',
    country: 'France',
    ...
    note: 'Became Queen of France & England during her life'
  },
  {
    id: 'I1341',
    country: 'France',
    ...
    name: 'Louis XIV',
    note:
      'The Sun King King was, with 72 years, the longest reigning monarch in European history'
  },
  {
    id: 'I65',
    country: 'United Kingdom',
    ...
    name: 'Diana',
    note: 'Hugely popular and loved, sadly killed in a car crash in 1997'
  },
  {
    id: 'I828',
    country: 'United Kingdom',
    ...
    name: 'Henry VIII',
    note: 'Had six wives, two of whom were executed'
  },
  {
    id: 'I3033',
    country: 'Monaco',
    title: 'Queen of Monaco',
    name: 'Grace Kelly',
    note: 'American actress who became a much loved Princess of Monaco'
  },
  {
    id: 'I1247',
    country: 'Scotland',
    title: 'Queen of the Scots',
    name: 'Mary Stuart',
    note: 'Acceded to the throne of Scotland six days after being born'
  },
  {
    id: 'I1088',
    country: 'Netherlands',
    title: 'Duchessof Nassau',
    name: 'Pauline of Wurttemberg',
    note:
      'Ancestress of the present Belgian, Danish, Dutch, Luxembourg, Norwegian, and Swedish Royal families'
  },
  {
    id: 'I21',
    country: 'Germany',
    ...
    name: 'William II',
    note: 'The last German & Prussian Emperor'
  },
  {
    id: 'I1122',
    country: 'Italy',
    title: 'King of Italy',
    name: 'Umberto II',
    note: 'The last King of Italy'
  },
  {
    id: 'I1196',
    country: 'Austria',
    title: 'Queen of Austria',
    name: "Elisabeth 'Sissi'",
    note: 'Was considered one of the greatest beauties of her time'
  },
  {
    id: 'I3086',
    country: 'Austria',
    title: 'Heir to Austria',
    name: 'Franz Ferdinand',
    note: 'His assassination was the last drop that started World War I'
  },
  {
    id: 'I643',
    country: 'Austria',
    ...
    name: 'Karl I',
    note: 'The last Emperor/King of Austria & Hungary'
  },
  {
    id: 'I3099',
    country: 'Portugal',
    ...
    name: 'Manuel II',
    note: 'The last King of Portugal'
  },
  {
    id: 'I1',
    country: 'United Kingdom',
    ...
    name: 'Victoria',
    note:
      "Known as 'the grandmother of Europe' after her 9 children married into royal and noble houses across Europe"
  },
  {
    id: 'I849',
    country: 'United Kingdom',
    title: 'Queen of England',
    name: 'Elizabeth I',
    note: 'The Virgin Queen was the last monarch of the Tudor dynasty'
  }
]
var royalsInteresting = interestingRoyal.map(function(d) {
  return d.id
})
...
var yearLoc = d3
  .range(0, height, height / yearSpread.length)
  .concat(height)
  .reverse()
...
var opacityScale = d3
  ...
  .domain([0, 3, 4, 6, 10])
  .clamp(true)
var opacityScaleHover = d3
  .scaleLinear()
  .range([1, 0.95, 0.7, 0.075])
  .domain([0, 1, 6, 7])
  .clamp(true)

var thicknessScaleHover = d3
  .scaleLinear()
  .range([2.5, 1.5])
  .domain([0, 6])
...
var voronoi = d3
  ...
  .extent([
    [-margin.left - width / 2, -margin.top],
    [width / 2 + margin.right, height + margin.bottom]
  ])
...
var tooltipWrapper = labelWrapper
  .append('g')
  .attr('class', 'tooltip-wrapper')
  .attr('transform', 'translate(' + 0 + ',' + 0 + ')')
  .style('opacity', 0)

var tooltipName = tooltipWrapper
  .append('text')
  .attr('class', 'tooltip-name')
  .text('Alex')

var tooltipTitle = tooltipWrapper
  .append('text')
  .attr('class', 'tooltip-title')
  .attr('y', 15)
  .text('King')

var tooltipExtra = tooltipWrapper
  .append('text')
...
labelWrapper
  ...
  .attr(
    'transform',
    'translate(' + spreadScale(-0.85) + ',' + yearScale(1990) + ')rotate(-90)'
  )
...
var royalTextWrapper = labelWrapper
  .selectAll('.royal-label')
  .data(currentRoyalLeaders)
  .enter()
  .append('g')
  .attr('class', 'royal-label')
  .attr('transform', function(d, i) {
    return 'translate(' + spreadScale(i) + ',' + yearScale(2020) + ')'
  })
...
var interestingRoyalTextWrapper = labelWrapper
  .selectAll('.interesting-royal-label')
...
var clickedCircle = svg
  .append('circle')
  .attr('class', 'node-clicked')
  .attr('r', 16)
  .style('opacity', 0)

var clickedCircleEnd = svg
  .append('circle')
  .attr('class', 'node-clicked')
  .attr('r', 16)
  .style('opacity', 0)
...
var firstClickNodeWrapper = labelWrapper
  .append('g')
  .attr('class', 'tooltip-wrapper')
  .attr('transform', 'translate(' + 0 + ',' + 0 + ')')
  .style('opacity', 0)

var firstClickedName = firstClickNodeWrapper
  .append('text')
  .attr('class', 'tooltip-name')
  .text('')

var firstClickedTitle = firstClickNodeWrapper
  .append('text')
  .attr('class', 'tooltip-title')
  .attr('y', 15)
  .text('')
...
var pathClickNodeWrapper = labelWrapper
  .append('g')
  .attr('class', 'tooltip-wrapper')
  .attr('transform', 'translate(' + 0 + ',' + 0 + ')')
  .style('opacity', 0)

var pathClickedName = pathClickNodeWrapper
  .append('text')
  .attr('class', 'tooltip-name')
  .text('')

var pathClickedTitle = pathClickNodeWrapper
  .append('text')
  .attr('class', 'tooltip-title')
  .attr('y', 15)
  .text('')
...
var nodesSave, linkSave

Promise.all([
  d3.json('royal-families-links-force.json'),
  d3.csv('royal-families-members-force.csv')
]).then(data => {
  let links = data[0]
  let nodes = data[1]
  draw('', links, nodes)
...
function draw(error, links, nodes) {
  ...
  links.forEach(function(d) {
    ...
    //Find a good radius
    d.r =
      Math.sqrt(sq(d.target.x - d.source.x) + sq(d.target.y - d.source.y)) * 2
  ...
  nodes.forEach(function(d) {
    ...
    d.radius = 3
    if (royals.indexOf(d.id) > -1) {
      d.radius = 10
    }
    if (royalsInteresting.indexOf(d.id) > -1) {
      d.radius = 6
    }

    d.fill = colorScale(d.min_dist_to_royal)
    if (royalsInteresting.indexOf(d.id) > -1) {
      d.fill = interestingRoyalColor
    }
...
function drawNodes(nodes, opacity, fill) {
  nodes.forEach(function(d) {
    ...
    ctxNodes.shadowBlur =
      royals.indexOf(d.id) > -1 || royalsInteresting.indexOf(d.id) > -1
        ? 30
        : 15
