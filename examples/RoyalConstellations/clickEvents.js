...
function clickedOnNode(d) {
  ...
  if (!clickLocked) {
    ...
    firstClickNodeWrapper
      ...
      .attr(
        'transform',
        'translate(' + d.x + ',' + (d.y - labelOffset - 15) + ')'
      )
...
function highlightRoute(path) {
  ...
  var nodesInPath = path.map(function(d) {
    return d.source
  })
  ...
  nodesSave.forEach(function(d) {
    ctxNodes.globalAlpha =
      nodesInPath.indexOf(d.id) > -1 ? opacityScaleHover(distanceNum) : 0.1
    var col =
      nodesInPath.indexOf(d.id) > -1
        ? colorScale(distanceNum)
        : colorScale(1000)
  ...
  labelWrapper
    .selectAll('.royal-label, .interesting-royal-label')
    .transition()
    .duration(200)
    .style('opacity', function(l, i) {
      return nodesInPath.indexOf(l.id) > -1 ? 1 : 0.1
    })
} //highlightRoute
