...
const nameAccessor = (nodes, id) => {
  const temp = nodes.find((item) => item.id === id)
  return temp.name
}

async function getData() {
  const { data } = await d3.json('data.json')
  dataset = data.links.map((link) => {
    return {
      source: nameAccessor(data.nodes, link.source),
      target: nameAccessor(data.nodes, link.target),
      value: link.value,
    }
  })
  console.log(dataset)
...
function draw(matrix, names, nodes) {
